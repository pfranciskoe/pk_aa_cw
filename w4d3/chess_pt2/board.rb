...
class Board
  ...
  def in_check?(color)
    ...
    @rows.each do |row|
      row.each do |ele|
        return true if ele.color != color && ele.moves.include?(king_position)
      end
    end
    false
  end
  ...
  def checkmate?(color)
    king = @board[king_pos(color)]
    if in_check?(color)
      if king.moves.all? do |mvs|
        @rows.any? do |row|
          row.any? do |ele|
            ele.color != color && ele.moves.include?(mvs)
          end
        end
      end
      end
